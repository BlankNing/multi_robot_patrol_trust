...
def calculate_features(df, reporter_id, provider_id, current_time, window_size):
    ...
    # Average rating
    avg_rating = relevant_interactions['rating_to_provider'].mean()
...
features_df.to_csv(f'./derived_features_as_reporter_{env_number}.csv', index=False)

print(f"Features derived and saved to 'derived_features_as_reporter_{env_number}.csv'.")